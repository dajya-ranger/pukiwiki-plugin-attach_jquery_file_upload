 ...
 * @since 	2020/08/14 外部呼び出しEXIT要求による処理分岐・添付ファイルリスト出力プラグイン組み込み
 *
 */
...
// 添付ファイルドラッグ＆ドロップアップロード対応プラグインファイル読み込み
...
function catbody($title, $page, $body)
{
	...
	/* 外部呼び出しEXIT要求定数が設定されているかを条件に含める
	if (!$is_read || $nofollow) {
	*/
	if ((!$is_read || $nofollow) && !defined('EXTERNAL_CALL_REQUIRE_EXIT')) {
		if (!headers_sent()) {
	...
	*/
	if ($is_read && !headers_sent() && !defined('EXTERNAL_CALL_REQUIRE_EXIT')) {
	...
	// 外部呼び出しEXIT要求の場合はスキンファイルを出力せずに戻る
...
$s_tpages
  ...
  <input type="submit" name="template" value="$_btn_load" id="template_read" accesskey="r" />
  <br />
EOD;
...
EOD;
	...
	// 添付ファイルリスト＋ドラッグ＆ドロップアップロードスクリプト出力
	return $body. attach_form_dnd($s_page, TRUE);
}
